...
# creating a new object from og data set
...
# base layer
ggplot(data = abalone_clean, # df as abalone_clean
...
**The missing value comes from a pair of tanks in which the pH sensor maulfunctioned, So there is no pH reading from those tanks**  
...
`x`, `predicted`, `std_error`, `conf.low`, `conf.high`, `group` (not going to worry about `group` column for now)
